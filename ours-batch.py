...
class NAR(BaseEstimator, TransformerMixin):
	...
	def _init_graph(self):
		...
		with self.graph.as_default():  # , tf.device('/cpu:0'):
			...
			interaction_rep = tf.reduce_prod(ui_rep, axis=1, keep_dims=True) # (None, 1, hidden_factor)
	...
	def _adding_attention(self, rep, feature_rep):
		...
		with tf.name_scope("adding_attention"):
			...
			exp_sum = tf.reduce_sum(att_exp, axis=1, keep_dims=True) # (None, 1)
	...
	def _initialize_weights(self):
		all_weights = dict()
		# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		all_weights['ui_embeddings'] = tf.Variable(tf.truncated_normal([self.ui_M, self.hidden_factor], 0.0, 0.1), # users_M * hidden_factor
													name='ui_embeddings', dtype=tf.float32)
		all_weights['node_embeddings'] = tf.Variable(tf.truncated_normal([self.nodes_M, self.hidden_factor], 0.0, 0.1), # nodes_M * hidden_factor
