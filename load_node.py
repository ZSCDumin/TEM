...
class NodeGenerator(object):
	...
	def _generate_train_batch(self, batch_size, batch_neg_sample):
		...
		all_pos_index = np.array(range(all_instances))[0::self.all_neg_sampling+1]

		per_batch_size = batch_size / (batch_neg_sample + 1)
		all_loop = len(all_pos_index) / per_batch_size
		all_batchs = np.zeros([all_loop, batch_size])

		for i in range(all_loop):
			pos_batch = np.array(random.sample(all_pos_index, per_batch_size))
			tmp_batch = pos_batch
